...
const createSessionCrisisDebugger = () => {
  const debugger = {
    ...
    description: 'Emergency diagnostics for Extension 2 session monitoring flood',
    ...
    diagnoseProblem() {
      ...
      const extensionScripts = allScripts.filter(script => 
        script.src && (script.src.includes('extension') || script.src.includes('roam'))
      );
      console.log(`📊 Found ${extensionScripts.length} extension-related scripts loaded`);
      ...
      if (window._extensionRegistry) {
        ...
        allListeners.forEach(listener => {
          ...
          bySource[listener.source || 'unknown'] = (bySource[listener.source || 'unknown'] || 0) + 1;
        });
        ...
        const sessionListeners = allListeners.filter(l => 
          l.source === 'session-monitoring' || 
          l.listener.toString().includes('updateActivity')
        );
        console.log(`🚨 Session monitoring listeners: ${sessionListeners.length}`);
      ...
      if (window.RoamExtensionSuite) {
        ...
        console.log('🔧 Available utilities:', Array.from(window.RoamExtensionSuite.utilities.keys()));
      } else {
    ...
    emergencyCleanup() {
      ...
      if (window._extensionRegistry && window._extensionRegistry.domListeners) {
        ...
        const sessionListeners = registry.domListeners.filter(l => 
          l.source === 'session-monitoring' || 
          l.listener.toString().includes('updateActivity')
        );
        
        console.log(`🎯 Found ${sessionListeners.length} session monitoring listeners to remove`);
        
        sessionListeners.forEach(listenerInfo => {
          try {
            document.removeEventListener(listenerInfo.type, listenerInfo.listener, true);
            cleanedCount++;
            console.log(`🧹 Removed ${listenerInfo.type} listener`);
          } catch (error) {
            console.warn(`⚠️ Failed to remove ${listenerInfo.type} listener:`, error.message);
          }
        });
        
        // Update registry
        registry.domListeners = registry.domListeners.filter(l => 
          l.source !== 'session-monitoring' && 
          !l.listener.toString().includes('updateActivity')
        );
        
        console.log(`🧹 Registry cleanup: ${originalCount} → ${registry.domListeners.length} listeners`);
      }
      ...
      console.log(`✅ Emergency cleanup completed. Removed ${cleanedCount} session listeners`);
    ...
    repairExtension2() {
      ...
      if (window.RoamExtensionSuite && window.RoamExtensionSuite.has('user-authentication')) {
        console.log('✅ Extension 2 is registered with platform');
        
        // Try to get the session
        const getAuthenticatedUser = window.RoamExtensionSuite.getUtility('getAuthenticatedUser');
        ...
        const getSessionInfo = window.RoamExtensionSuite.getUtility('getSessionInfo');
        if (getSessionInfo) {
          try {
    ...
    startRealTimeMonitoring() {
      ...
      console.error = function(...args) {
        ...
        if (errorMsg.includes('updateActivity')) {
          errorTypes['updateActivity'] = (errorTypes['updateActivity'] || 0) + 1;
          
          // Only log every 10th error to reduce spam
          if (errorTypes['updateActivity'] % 10 === 1) {
            originalError('🚨 Session monitoring error #' + errorTypes['updateActivity'], ...args);
          }
        } else {
          originalError(...args);
        }
      ...
      console.log('📊 Will report every 10th updateActivity error to reduce spam');
      
      // Report summary every 10 seconds
      const reportInterval = setInterval(() => {
        if (errorCount > 0) {
          console.log(`📊 Error summary (last 10s): ${errorCount} total, updateActivity: ${errorTypes['updateActivity'] || 0}`);
          errorCount = 0;
    ...
    quickStatus() {
      ...
      console.log(`📊 Registry listeners: ${registry?.domListeners?.length || 0}`);
      console.log(`📊 Session listeners: ${
        registry?.domListeners?.filter(l => 
          l.source === 'session-monitoring' || 
          l.listener.toString().includes('updateActivity')
        ).length || 0
      }`);
      console.log(`🎯 Platform status: ${platform ? 'Available' : 'Missing'}`);
      console.log(`🎯 Extension 2 status: ${platform?.has('user-authentication') ? 'Registered' : 'Missing'}`);
      
      try {
        console.log(`👤 UserSession global: ${typeof userSession !== 'undefined' ? 'Available' : 'Missing'}`);
      } catch (error) {
...
const createEmergencyDebugInterface = () => {
  ...
  console.log('💡 Use the red panel on the right to diagnose and fix the session crisis');
};
...
export default {
  onload: async ({ extensionAPI }) => {
    ...
    // ✅ VERIFY FOUNDATION
    if (!window.RoamExtensionSuite) {
      console.error('❌ Foundation Registry not found! Creating emergency interface anyway...');
    }
      ...
      },
      {
        ...
        callback: () => {
          if (window.emergencyDebugger) {
            ...
            console.log('Emergency debugger not ready - run "EMERGENCY: Show Debug Platform" first');
          }
        }
      },
      {
    ...
    if (window.RoamExtensionSuite) {
      window.RoamExtensionSuite.register('emergency-debug-platform', {
        ...
        description: 'Crisis management tools for extension debugging emergencies',
        version: '0.1.0',
        dependencies: []
      });
    }
    ...
    console.log('💡 Step 1: Click "🔍 Diagnose Problem" to understand the crisis');
  },
