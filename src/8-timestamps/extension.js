...
let retryTimer = null;
let registeredCommands = []; // Track command palette commands
  ...
  switch (failureType) {
    case "no-uid":
      ...
      element.setAttribute(
        "title",
        "Could not determine block identifier. Click to retry or use Command Palette > 'Timestamp Pills: Force Refresh All'"
      );
      break;
    case "no-time":
      ...
      element.setAttribute(
        "title",
        "Could not determine creation time. Click to retry or use Command Palette > 'Timestamp Pills: Force Refresh All'"
      );
      break;
    case "api-not-ready":
      ...
      element.setAttribute(
        "title",
        "Roam API not ready. Click to retry or use Command Palette > 'Timestamp Pills: Force Refresh All'"
      );
      break;
    case "max-retries":
      ...
      element.setAttribute(
        "title",
        `${message}. Click to retry or use Command Palette > 'Timestamp Pills: Force Refresh All'`
      );
      break;
    default:
      ...
      element.setAttribute(
        ...
        }. Click to retry or use Command Palette > 'Timestamp Pills: Force Refresh All'`
      );
  }
...
const retryAllFailed = () => {
  ...
  failedElements.forEach((element) => {
    const failureType = element.getAttribute("data-failure-type") || "unknown";
  ...
  try {
    ...
    if (!createTime) {
      ...
      if (isRetry) {
        ...
        addToRetryQueue(tagElement, "no-time", "Block creation time not found");
      }
      return;
    }
...
const injectTimestampStyles = () => {
  ...
  styleElement.textContent = `
    ...
    /* 🌳 Error state - Light Red with click cursor */
    ...
    /* 🌳 Error state hover - Encourage clicking to retry */
    .rm-page-ref[data-tag="${config.tagName}"][data-time-class="ts-error"]:hover::before,
    a[data-link-title="${config.tagName}"][data-time-class="ts-error"]:hover::before,
    .roam-tag[data-tag="${config.tagName}"][data-time-class="ts-error"]:hover::before {
...
const setupObserver = () => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const hasTimestampTags =
              ...
              node.querySelector?.(`a[data-link-title="${config.tagName}"]`) ||
              node.matches?.(`a[data-link-title="${config.tagName}"]`) ||
              node.querySelector?.(`.roam-tag[data-tag="${config.tagName}"]`) ||
  ...
  // Set up click handlers for failed pills
  targetNode.addEventListener("click", (event) => {
    const clickedPill = event.target.closest(
      `[data-tag="${config.tagName}"][data-ts0-processed="error"]`
    );
    if (clickedPill) {
      ...
      debug("User clicked failed pill, retrying...", clickedPill);

      // Clear the error state and retry
      const failureType =
        clickedPill.getAttribute("data-failure-type") || "click-retry";
      clickedPill.removeAttribute("data-ts0-processed");
      clickedPill.removeAttribute("data-failure-type");

      // Add to retry queue
      addToRetryQueue(clickedPill, failureType, "user click retry");
    }
  });
...
const registerCommands = () => {
  if (!window.roamAlphaAPI?.ui?.commandPalette?.addCommand) {
    debug("Command palette not available");
    return [];
  }

  const commands = [
    {
      label: "Timestamp Pills: Force Refresh All",
      callback: () => {
        debug("Command palette: Force refresh triggered");
        forceRefresh();
      },
    },
    {
      label: "Timestamp Pills: Retry Failed Only",
      callback: () => {
        debug("Command palette: Retry failed triggered");
        retryAllFailed();
      },
    },
    {
      label: "Timestamp Pills: Show Status",
      callback: () => {
        const status = getRetryQueueStatus();
        const failedCount = document.querySelectorAll(
          `[data-tag="${config.tagName}"][data-ts0-processed="error"]`
        ).length;
        const successCount = document.querySelectorAll(
          `[data-tag="${config.tagName}"][data-ts0-processed="success"]`
        ).length;

        console.log("=== Timestamp Pills Status ===");
        console.log(`✅ Successful: ${successCount}`);
        console.log(`❌ Failed: ${failedCount}`);
        console.log(`🔄 In retry queue: ${status.queueSize}`);
        console.log(`📊 Total processed: ${successCount + failedCount}`);

        if (failedCount > 0) {
          console.log(
            "\n💡 Run 'Timestamp Pills: Force Refresh All' to retry failed pills"
          );
        }
      },
    },
  ...
  commands.forEach((cmd) => {
    try {
      ...
      registeredCommands.push(cmd.label);
      debug(`✅ Registered command: ${cmd.label}`);
    } catch (error) {
      debug(`❌ Failed to register command ${cmd.label}: ${error.message}`);
    }
  });
...
const unregisterCommands = (commandLabels) => {
  ...
  commandLabels.forEach((label) => {
    try {
      ...
      debug(`✅ Unregistered command: ${label}`);
    } catch (error) {
      debug(`❌ Failed to unregister command ${label}: ${error.message}`);
    }
  });
};
...
export default {
  onload: ({ extensionAPI }) => {
    ...
    // Set up API ready detection for automatic retry
    if (!isRoamApiReady()) {
      debug("⏳ Roam API not ready, setting up detection...");
      const apiReadyCheck = setInterval(() => {
        if (isRoamApiReady()) {
          debug(
            "✅ Roam API is now ready! Triggering automatic retry of failed pills..."
          );
          clearInterval(apiReadyCheck);

          // Wait a moment for API to fully stabilize
          setTimeout(() => {
            retryAllFailed();
          }, 1000);
        }
      ...
      setTimeout(() => {
        clearInterval(apiReadyCheck);
        debug("⚠️ Stopped waiting for Roam API after 30 seconds");
      }, 30000);
    }
    ...
    debug(
      "🎯 Command Palette: 'Timestamp Pills: Force Refresh All', 'Timestamp Pills: Retry Failed Only', 'Timestamp Pills: Show Status'"
    );
    debug("👆 Click failed pills (red ones) to retry them individually");
  },
